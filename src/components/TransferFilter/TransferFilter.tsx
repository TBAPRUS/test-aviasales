...
export default function TransferFilter() {
  ...
  const all = values.reduce(
    (acc: boolean, cur: TransferFilterItem) => acc && cur.checked,
    true
  );
  ...
  return (
    <aside className={styles.container}>
      ...
      <ul>
        <li
          className={styles.li}
          onClick={() => dispatch(changeAllTransfers(!all))}
        >
          <input name="Все" type="checkbox" checked={all} readOnly />
          <label htmlFor="Все">Все</label>
        </li>
        {values.map((item: TransferFilterItem) => (
          <li
            key={item.name}
            onClick={() => dispatch(changeTransfers(item.name))}
            className={styles.li}
          >
            <input
              name={item.name}
